	...
	public PagerData<SaleOrder> pageDivider(SaleOrderSearch saleoOrderSearch) {
		...
		if (saleoOrderSearch != null) {
			...
			if (!StringUtils.isEmpty(saleoOrderSearch.getKeyword())) {
				sql += " and (s.title like '%" + saleoOrderSearch.getKeyword() + "%'" + 
						     " or s.customer_address like '%" + saleoOrderSearch.getKeyword() + "%'" + 
						     " or s.customer_name like '%" + saleoOrderSearch.getKeyword() + "%')";
			}
		}
